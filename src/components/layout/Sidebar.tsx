...
import {
  LayoutDashboard, Wallet, FileText, Repeat, ShoppingCart, PiggyBank, CreditCard, Settings, TrendingUp, ChevronLeft, ChevronRight,
} from "lucide-react";
...
const Sidebar: React.FC<SidebarProps> = ({ isCollapsed, onToggle }) => {
  return (
    <aside className={cn("fixed left-0 top-0 z-40 h-screen bg-sidebar border-r border-sidebar-border transition-all duration-300 ease-in-out flex flex-col", isCollapsed ? "w-16" : "w-64")}>
      ...
      <nav className="flex-1 px-2 py-4 space-y-1 overflow-y-auto">
        {menuItems.map((item) => (
          <NavLink
            ...
            className={({ isActive }) =>
              cn("flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all duration-200 group",
          ...
          >
            <item.icon className={cn("w-5 h-5 shrink-0 transition-transform duration-200 group-hover:scale-110")} />
      ...
      <div className="px-2 py-4 border-t border-sidebar-border space-y-1">
        ...
        <Button variant="ghost" size="sm" onClick={onToggle} className={cn("w-full justify-center text-muted-foreground hover:text-foreground", !isCollapsed && "justify-start px-3")}>
          {isCollapsed ? <ChevronRight className="w-5 h-5" /> : <><ChevronLeft className="w-5 h-5" /><span>Réduire</span></>}
        </Button>
      </div>
    </aside>
  );
